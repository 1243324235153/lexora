...
import { Scale, ArrowRight, Building2, Mail, User, Lock, AlertCircle } from "lucide-react";
...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("Kunde inte skapa kontot. Kontrollera att lösenordet är minst 6 tecken.");
      console.error(err);
    } finally {
      setIsLoading(false);
    }
      ...
      <main className="flex-1 flex items-center justify-center p-6">
        ...
        >
          <div className="text-center mb-8">
            <h1 className="text-3xl font-serif text-lexora-text mb-3">Skapa konto</h1>
            <p className="text-lexora-text/70">
              Börja använda Lexora som ert juridiska beslutsstöd idag.
            </p>
          </div>
          ...
          {!isFirebaseConfigured && (
            <div className="mb-6 bg-amber-50 border border-amber-200 text-amber-800 rounded-xl p-4 flex items-start gap-3 text-sm">
              ...
              <div>
                ...
                <p>För att registrering ska fungera måste du konfigurera Firebase-miljövariablerna i AI Studio.</p>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="block text-sm font-medium text-lexora-text mb-2">Fullständigt namn</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                  <User className="h-5 w-5 text-lexora-text/40" />
                </div>
                <input
                  type="text"
                  required
            ...
            <div>
              <label className="block text-sm font-medium text-lexora-text mb-2">Företag</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                  <Building2 className="h-5 w-5 text-lexora-text/40" />
                </div>
                <input
                  type="text"
                  required
            ...
            >
              {isLoading ? (
                ...
                <>
                  Registrera konto <ArrowRight className="w-5 h-5" />
                </>
              )}
            </motion.button>
          </form>
          ...
          <div className="mt-8 text-center text-sm text-lexora-text/60">
            Har du redan ett konto? <Link to="/login" className="text-lexora-text font-medium hover:underline">Logga in</Link>
          </div>
        </motion.div>
      </main>
    </motion.div>
